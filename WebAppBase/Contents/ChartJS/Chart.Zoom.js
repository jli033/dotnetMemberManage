...
function zoomIndexScale(scale, zoom, center, zoomOptions,chartInstance) {
	...
	if (sensitivity < 3) {
	    ...
	    var startx = Math.max(0, (chartInstance._dragZoomStart.x - $(chartInstance.chart.canvas).position().left - chartInstance.chartArea.left));
	    var endx = Math.max(0, (chartInstance._dragZoomEnd.x - $(chartInstance.chart.canvas).position().left - chartInstance.chartArea.left));

	    var startxP = startx / (chartInstance.chart.width - chartInstance.chartArea.left);
	    var endxP = endx / (chartInstance.chart.width - chartInstance.chartArea.left);
	    //console.log(startxP * 100);
	    //console.log(endxP * 100);
	    var indexCount = maxIndex - minIndex+1;
	    var leftIdx = parseInt(indexCount * startxP) + minIndex;
	    var rightIdx = parseInt(indexCount * endxP) + minIndex;
	    minIndex = Math.min(leftIdx, rightIdx);
	    maxIndex = Math.max(leftIdx, rightIdx);
...
function zoomScale(chartInstance, scale, zoom, center, zoomOptions) {
	var fn = zoomFunctions[scale.options.type];
	if (fn) {
	    fn(scale, zoom, center, zoomOptions,chartInstance);
	}
}
	...
	if (zoomOptions && helpers.getValueOrDefault(zoomOptions.enabled, defaultOptions.zoom.enabled)) {
		...
		helpers.each(chartInstance.scales, function(scale, id) {
			if (scale.isHorizontal() && directionEnabled(zoomMode, 'x')) {
			    zoomScale(chartInstance, scale, zoom, center, zoomOptions);
...
var zoomPlugin = {
		...
		if (options.zoom.drag) {
			...
			node.addEventListener('mouseup', function(event){
				if (chartInstance._dragZoomStart) {
					...
					var drawStartX = startX - $(chartInstance.chart.canvas).position().left;// - chartInstance.chartArea.left;
					if (dragDistance > 0) {
						doZoom(chartInstance, zoom, {
						    x: (dragDistance / 2) + drawStartX,
	...
	beforeDatasetsDraw: function(chartInstance) {
		...
		if (chartInstance._dragZoomEnd) {
			...
			var drawStartX = startX - $(chartInstance.chart.canvas).position().left;
			ctx.fillStyle = 'rgba(255,225,153,0.3)';
			ctx.lineWidth = 5;
			ctx.fillRect(drawStartX, yAxis.top, rectWidth, yAxis.bottom - yAxis.top);
		}
